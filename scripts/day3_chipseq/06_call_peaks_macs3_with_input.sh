...
tail -n +2 "$SAMPLES" | while IFS=$'\t' read -r sample_id condition assay role fq1 fq2 input_sample rest; do
  ...
  [[ -n "${input_sample:-}" && "$input_sample" != "NA" ]] || die "Input sample missing for $sample_id"
  need_file "$BAM_DIR/${input_sample}.sorted.bam"
  macs3 callpeak -t "$BAM_DIR/${sample_id}.sorted.bam" -c "$BAM_DIR/${input_sample}.sorted.bam" -f BAM -g "$GENOME_SIZE" -n "$sample_id" --outdir "$OUTDIR" --keep-dup all 2>&1 | tee "$LOGDIR/08_macs3_with_input_${sample_id}.log"
done
