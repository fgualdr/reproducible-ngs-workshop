...
write.table(normalized_out, file.path(out_dir, "normalized_counts.tsv"),sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(scale_dt, file.path(out_dir, "deseq2_size_factors.tsv"),sep = "\t", quote = FALSE, row.names = FALSE)
...
# several options:
normalized_counts_s <- (normalized_counts +1) / rowMeans(normalized_counts+1)
normalized_counts_s <- normalized_counts  - rowMeans(normalized_counts)
