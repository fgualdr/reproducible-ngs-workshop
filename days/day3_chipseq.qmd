...
::: {.callout-warning title="Published analysis ≠ teaching analysis"}
...
The workshop retains only wild-type-genotype samples, maps them to `GCF_025998455.1` (`NZ_AP026446.1`), calls each sample with MACS3 without an external control, merges overlapping calls into a permissive Day 4 peak set, counts BAM overlaps, and uses DESeq2 for a condition comparison. This teaches the analysis logic; it is not expected to reproduce the paper's peak coordinates, significance values, or biological conclusions.
:::
...
For this workshop we use MACS3 without an external control because it keeps the practical small and consistent. MACS3 can call peaks this way by estimating its background from the ChIP/treatment data, including a large local window and a genome-wide component when no control is supplied. This makes no-control calling usable for an explicitly exploratory teaching analysis; it is not equivalent to measuring background in a matched input or target-negative library. Section 11 explains the tradeoff.
...
Day 4 needs one stable peak file to integrate with RNA-seq and annotation. Instead of choosing one replicate, we create a simple peak set by merging overlapping MACS3 peaks across the samples. Here, “consensus” means a **permissive union**: every region called in at least one sample enters the merged set, and no replicate-support threshold is applied. This transparent teaching handoff is not the publication's `csaw`/`edgeR` region set.
...
The R script selects the R1 rows from `Chipseq_metadata.txt` so that each biological sample appears once. It obtains sample names and replicate numbers from the SDRF, recodes `none` as `condition_A` and `21 percent oxygen` as `condition_B`, and matches those sample names to the peak-count columns before fitting DESeq2.

This is a simplified teaching analysis. The `bedtools multicov` matrix counts overlapping alignments; a publication-grade paired-end analysis should use fragment-aware counting and should model the complete experimental design. This DESeq2 result is a condition comparison within the workshop union peak set, not the paper's `csaw`/`edgeR` sliding-window analysis. Keep raw counts—not CPM-normalized bigWig signal—as the DESeq2 input.
...
| No external control | Background inferred from the ChIP/treatment library | Feasible for this teaching exercise and some exploratory or pilot analyses | Cannot independently identify systematic ChIP-library background |

For this workshop, no-control MACS3 is acceptable as an **explicitly labeled exploratory teaching route**. We compensate in interpretation—not statistically—by checking replicate concordance, coverage tracks, peak shape and score, motif plausibility, and selected loci. A research study should choose controls during experimental design and validate key binding claims orthogonally.
